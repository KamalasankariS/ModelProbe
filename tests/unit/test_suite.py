"""Unit tests for run_suite and assert_eval."""
...
def test_all_pass(noop_backend):
...
def test_all_fail(noop_backend):
...
def test_mixed_results(noop_backend):
    cases = [
...
def test_runner_exception_counts_as_error(noop_backend):
...
def test_suite_result_shape(noop_backend):
    result = run_suite(
        suite_name="my-suite",
        version="v3",
